...
/*-----------CALCULAR SI NECESITA RECAMBIO DENTRO DE 3 DÍAS--------------------*/
    SELECT FLOOR((horas+20)/tiempo_vida) INTO cantidad;
...
/*----------VERIFICAR SI SE REQUIERE CAMBIAR EL COMPONENTE---------------------*/
    IF(cantidad > 0) THEN
        ...
        /*-------------CAMBIAR COMPONENTE-----------------------*/
        INSERT INTO work_order_details(work_order_id,task_id,component_implement_id,state,created_at,updated_at) VALUES (orden_trabajo,tarea_componente,implemento_componente,'RECOMENDADO',NOW(),NOW());
    ELSE
        ...
        BEGIN
            ...
            OPEN cur_task;
                ...
                IF NOT EXISTS(SELECT * FROM work_order_details WHERE state = "ACEPTADO" AND work_order_id = orden_trabajo AND task_id = tarea_componente) THEN
        ...
        BEGIN
            /*------------CURSOR PARA PIEZAS------------------------*/
            DECLARE cur_part CURSOR FOR SELECT cpm.part,c.lifespan,c.item_id,it.estimated_price FROM component_part_model cpm INNER JOIN components c ON c.id = cpm.part INNER JOIN component_implement_model cim ON cim.id = cpm.component INNER JOIN items it ON it.id = c.item_id WHERE cpm.component = componente;
            ...
            OPEN cur_part;
                ...
                /*-------------CALCULAR SI NECESITA RECAMBIO DENTRO DE 3 DÍAS---------------------*/
                SELECT FLOOR((horas_pieza+20)/tiempo_vida_pieza) INTO cantidad_pieza;
                /*----------VERIFICAR SI SE REQUIERE CAMBIAR EL COMPONENTE----------------------------*/
                IF(cantidad_pieza > 0) THEN
                    ...
                    /*-------------CAMBIAR COMPONENTE--------------------------------------------------*/
                    INSERT INTO work_order_details(work_order_id,task_id,state,component_part_id,created_at,updated_at) VALUES (orden_trabajo,tarea_componente,'RECOMENDADO',componente_pieza,NOW(),NOW());
                ELSE
                    /*------------RUTINARIO DE PIEZAS-----------------------------*/
                    BEGIN
                        /*------------CURSOR PARA ITERAR TAREAS POR PIEZA-----------------*/
                        DECLARE cur_task_part CURSOR FOR SELECT id FROM tasks WHERE component_id = pieza AND task <> "RECAMBIO";
                        DECLARE CONTINUE HANDLER FOR NOT FOUND SET tarea_final = 1;
                        ...
                        OPEN cur_task_part;
                            ...
                            END IF;
                            FETCH cur_task_part INTO tarea_pieza;
                            IF NOT EXISTS(SELECT * FROM work_order_details WHERE state = "ACEPTADO" AND work_order_id = orden_trabajo AND task_id = tarea_pieza) THEN
