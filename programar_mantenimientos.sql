            ...
            BEGIN
                ...
                OPEN cursor_componentes;
                    bucle_componentes:LOOP
                        ...
                        /*---------------OBTENER FRECUENCIA DE MANTENIMIENTO PREVENTIVO-----------------------------------*/
                        SELECT frequency INTO frecuencia_componente FROM preventive_maintenance_frequencies WHERE component_id = componente;
                        /*--------------OBTENER HORAS ÚLTIMO MANTENIMIENTO EN CASO HUBIERA-------------------------------------------------*/
                        IF EXISTS(SELECT * FROM work_order_details wod WHERE wod.component_implement_id = componente_del_implemento) THEN
                            SELECT wod.component_hours INTO horas_ultimo_mantenimiento_componente FROM work_order_details wod WHERE wod.component_implement_id = componente_del_implemento AND wod.is_checked = 1 ORDER BY id DESC LIMIT 1
                        ELSE
                        ...
                        IF(cantidad_componente > 0) THEN
                            ...
                            SELECT id INTO tarea FROM tasks WHERE component_id = componente AND type = "RECAMBIO" LIMIT 1;
                            ...
                            END IF;
                        /*---------------TAREA DE MANTENIMIENTO SI LO NECESITA-----------------------------------------*/
                        ELSE IF (horas_ultimo_mantenimiento_componente+frecuencia_componente <= horas_componente)
                            100 + 300 = 400
                            350  -  500
                        /*----------------RUTINARIO SI NO NECESITA RECAMBIO NI MANTENIMIENTO----------------------------*/
                        ELSE
                            /*------------CURSOR PARA CREAR EL RUTINARIO POR CADA COMPONENTE----------------*/
                            BEGIN
                                DECLARE cursor_componente_tareas CURSOR FOR SELECT id FROM tasks WHERE component_id = componente AND type = "RUTINARIO";
                            ...
                            /*------------CURSOR PARA CREAR EL PREVENTIVO POR CADA COMPONENTE----------------*/
                            BEGIN
                                DECLARE cursor_componente_tareas CURSOR FOR SELECT id FROM tasks WHERE component_id = componente AND type = "PREVENTIVO";
                                DECLARE CONTINUE HANDLER FOR NOT FOUND SET tarea_final = 1;
                                /*-----------------------------------ABRIR CURSOR DE TAREAS DEL COMPONENTE------------------------------*/
                                OPEN cursor_componente_tareas;
                                    bucle_componente_tareas:LOOP
                                        ...
                                        /*----------------OBTENER TAREA DEL COMPONENTE-----------------------------------------------*/
                                        FETCH cursor_componente_tareas INTO tarea;
                                        /*-------------INSERTAR PREVENTIVO DE TAREAS EN EL DETALLE DE LA ORDEN DE TRABAJO-----------------------*/
                                        IF NOT EXISTS(SELECT * FROM work_order_details WHERE work_order_id = orden_trabajo AND task_id = tarea AND state = "ACEPTADO" AND component_implement_id = componente_del_implemento) THEN
                                            INSERT INTO work_order_details (work_order_id,task_id,component_implement_id ) VALUES (orden_trabajo,tarea,componente_del_implemento);
                                        END IF;
                                        IF EXISTS(SELECT * FROM task_required_materials WHERE task_id = tarea) THEN
                                ...
                                OPEN cursor_piezas;
                                    bucle_piezas:LOOP
                                        ...
                                        IF(cantidad_pieza > 0) THEN
                                            ...
                                            SELECT id INTO tarea FROM tasks WHERE component_id = pieza AND type = "RECAMBIO" LIMIT 1;
                                        ...
                                        ELSE
                                        /*--------------CURSOR PARA CREAR EL RUTINARIO DE LAS PIEZAS POR CADA COMPONENTE-----------------*/
                                            BEGIN
                                                DECLARE cursor_pieza_tareas CURSOR FOR SELECT id FROM tasks WHERE component_id = pieza AND type = "RUTINARIO";
                                                DECLARE CONTINUE HANDLER FOR NOT FOUND SET tarea_final = 1;
                                                /*--------ABRIR CURSOR DE TAREAS DEL COMPONENTE------------------------------*/
                                                ...
                                                SELECT 0 INTO tarea_final;
                                            END;
                                        /*--------------FIN RUTINARIO DE LAS PIEZAS POR CADA COMPONENTE-----------------------*/
                                        /*--------------CURSOR PARA CREAR EL PREVENTIVO DE LAS PIEZAS POR CADA COMPONENTE-----------------*/
                                            BEGIN
                                                DECLARE cursor_pieza_tareas CURSOR FOR SELECT id FROM tasks WHERE component_id = pieza AND type = "PREVENTIVO";
                                                DECLARE CONTINUE HANDLER FOR NOT FOUND SET tarea_final = 1;
                                                /*--------ABRIR CURSOR DE TAREAS DEL COMPONENTE------------------------------*/
                                                OPEN cursor_pieza_tareas;
                                                    bucle_pieza_tareas:LOOP
